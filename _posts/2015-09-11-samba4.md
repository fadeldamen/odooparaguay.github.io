...
--dns-backend=SAMBA_INTERNAL_DLZ --realm=empresa /root/backup/etc/samba/smb.conf
{% endhighlight %}
...
> Nota: Caso você tenha problemas com SID duplicados, por favor acessar a documentação do Samba em **[Prevent failure due to duplicate SID's](https://wiki.samba.org/index.php/Migrating_a_Samba_NT4_domain_to_a_Samba_AD_domain_(classic_upgrade)**. Isto é, lá contém um script em python que ajuda a resolver este problema.
