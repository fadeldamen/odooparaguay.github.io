...
* **[Promovendo o Samba](#promovendo-o-samba)**
...
Você poderá tentar em versões anteriores do qual estiver melhor famialiarizado. Estarei me baseando no Debian Jessie "8" amd64 no entanto:

Para instalar o Debian com conhecimento seguro, basta novamente acessar a documentação que explica passo-a-passo (inclusive em português do Brasil) - **[https://www.debian.org/releases/stable/amd64/](https://www.debian.org/releases/stable/amd64/)**.

Se deseja uma abordagem simplória dos passos de instalação, acompanhe a baixo:
...
A instalação do **smbclient** é opcional, utilizaremos apenas para testar as configurações adiante.
...
192.168.1.100 debian.qualquernome.com.br debian
{% endhighlight %}
...
dns-search qualquernome.com.br
{% endhighlight %}
...
domain qualquernome.com.br
search qualquernome.com.br
nameserver 192.168.1.100
nameserver 192.168.1.1
{% endhighlight %}

O nameserver 192.168.1.100 será utilizado pelo samba, enquanto o 192.168.1.1 é o serviço de DNS da rede local, pode ser modificado para outro de sua escolha (ex: 8.8.8.8 - google).

Configurar serviço de NTP (atualização de data/hora):
...
{% highlight bash %}
mv /etc/samba/smb.conf smb-apt-default.conf
{% endhighlight %}
...
{% highlight bash %}
samba-tool domain provision --use-rfc2307 --realm=QUALQUERNOME.COM.BR --domain=QUALQUERNOME --server-role=dc --dns-backend=SAMBA_INTERNAL --adminpass='S@mb4s3rveR!'
{% endhighlight %}
...
[global]
        workgroup = QUALQUERNOME
        realm = QUALQUERNOME.COM.BR
        netbios name = DEBIAN
...
Apenas desativei as impressoras, pois não utilizo. Adicionei um compartilhamento (home) para os usuários, ajuste este caminho para suas necessidades, conforme seu esquema de particionamento.
...
{% highlight bash %}
kinit administrator@QUALQUERNOME.COM.BR
{% endhighlight %}

Teste também as configurações de DNS do samba:

{% highlight bash %}
host -t SRV _ldap._tcp.qualquernome.com.br
host -t SRV _kerberos._udp.qualquernome.com.br
host -t A debian.qualquernome.com.br.
{% endhighlight %}

Teste a conectividade com o samba:

{% highlight bash %}
smbclient -k //debian.qualquernome.com.br/netlogon -c 'ls'
{% endhighlight %}
