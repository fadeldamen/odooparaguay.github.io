...
* **[Instalacao do Centos](#instalacao-do-centos)**
...
* **[Usando ACLs para permissoes avançadas no Linux](#usando-acls-para-permissoes-avancadas-no-linux)**
...
![Samba](https://raw.githubusercontent.com/lobocode/lobocode.github.io/master/media/sysvol.gif)
...
Por conveniência, irei trabalhar com Debian e CentOs por tratar-se das distribuições GNU/Linux mais usadas em servidores. Quanto à versão do Debian, é recomendado neste caso sempre a versão **Stable** visto que o ciclo de atualização do sistema Stable é diferente das versões unstable. Por exemplo, em sistemas stable, só haverãá atualização para softwares e sistema em sí, caso já tenham esgotado todos os possíveis problemas, bugs relacionados.

Já o Cent0S, é irrelevante a versão visto que o Cent0S por definição sempre é stable. Por outro lado, na maioria das vezes a comunidade recomenda o uso de versões mais maduras de cada sistema. Isto porque é comum algumas bibliotecas e softwares ainda não terem sido portados e\ou atualizados para as novas versões do sistema novo. Geralmente esse tipo de problema ocorre mai quando se é exigido um ambiente gráfico para trabalhar.

Para rodar bem o Cent0s ou Debian Stable, basta um computador simples (dispensa ambiente gráfico) podendo ser testado em ambiente virtual com virtualbox, vmware, qemu, ou xenserver.Pode ser testado inclusive em uma máquina antiga como um Pentium 4 ou com arquitetura inferior (bastando você fazer download do sistema na versão da arquitetura desejada). É desejoso que se tenha o mínimo de conhecimento com instalação e configuração do sistema também.

* **[Download Cent0s](https://www.centos.org/download/)** - costumo baixar a ISO (mas não é obrigatório).
* **[Download Debian Stable](https://www.debian.org/CD/http-ftp/)** - testado com a versão 8 com sucesso. 
...
{% highlight bash %} 
# Caso seu Centos seja o mais recente, basta apenas o comando a baixo 
rpm -Uvh http://dl.fedoraproject.org/pub/epel/7/x86_64/e/epel-release-7-5.noarch.rpm

# Caso prefira a versão anterior
...
Pós a configuração do hostname, o sistema pedirá que o usuário crie uma senha de usuário 'root'. Certifique-se de não esquecer esta senha, pois não é um processo divertido tentar recuperar a senha de root caso seja descuidado:
...
![SMB](https://raw.githubusercontent.com/lobocode/lobocode.github.io/master/media/debi5.png)
...
### Instalacao do Centos

No caso do CentOs, não se diferente tanto da instalação do Debian. A única diferença é que para quem está acostumado com o instalador debian (que geralmente é o mesmo usado no Ubuntu e outras distribuições baseadas no Debian), passará a instalar através do **Anaconda**.

> Nota: A documentação do CentOs para a instalação em modo texto é bastante pobre e acaba dando mais trabalho do que a em modo gráfico. Instalar em modo gráfico, não quer dizer que obrigatoriamente você terá uma interface de usuário ao fim da instalação. 

Afim de não tornar este processo **redundante** por já haver diversos tutoriais ensinando a instalar o CentOS passo-a-passo e com screenshot, colocarei a baixo dois links que ensina o processo passo-a-passo. Geralmente, lidar com o CentOs acaba sendo mais simples do que o Debian em muitas coisas. A instalação não é uma dessas coisas mas também não é nenhuma dor de cabeça:

* **[Tecmint - How to install CentOs 7?](http://www.tecmint.com/centos-7-installation/)**
* **[Hexblot - How to install CentOs 7?](http://www.hexblot.com/blog/centos-7-server-installation)**

> Nota: Preste bastante atenção quando chegar em **Software Selection**. Pois, é aí que você irá definir para que propósito está instalando o seu CentOs. Isto é, para servidor? para desktop? para infra-estrutura?geralmente eu coloco o **"minimal install"** pois vem apenas com o básico do CentOs e sem interface gráfica. Se você tiver dificuldade em instalar o CentOs, me mande um e-mail para que eu possa registrar aqui o passo-a-passo se necessário.
...
> Nota: Tanto para Debian quanto para CentOs iremos instalar todo o conteúdo a cima. A diferença é que para um estes pacotes serão mais explicitados na instalação e para outro, estarão como dependência que será automaticamente inserido durante a instalação de menos pacotes.
...
E altere sua configuração de modo parecido com o abaixo (não copie e cole esta linha pois, cada máquina tem uma configuração de UUID diferente):
...
> Nota: As opções acl e user_xattr são necessários, a fim de utilizar os recursos de arquivos POSIX. Preste atenção caso tenha definido o sistema de arquivos emLogical Volume Management (LVM). Pois, LVM não suporta a variável "barrier". O barrier serve para adicionar um sistema de segurança para com falhas de energia em formatações ext3 ou ext4.
...
> Nota: O parâmetro -o, vem de --option e o rw vem de raead/write. Consulte a manpage de cada comando para compreender do que se trata.
...
Configure agora a interface de rede:
...
> Nota: Para instalar o samba, bastava apenas ter digitado apt-get install samba como mostra no início. No entanto, preferí deixar algumas coisas prontas antes de prosseguir-mos. 
...
{% highlight bash %}
mv /etc/samba/smb.conf /etc/samba/smb-apt-default.conf
{% endhighlight %}
...
{% highlight bash %}
samba-tool domain provision --use-rfc2307 --realm=SEUDOMINIO.COM.BR \
--domain=SEUDOMINIO --server-role=dc --dns-backend=SAMBA_INTERNAL \
--adminpass='S@mb4s3rveR!'
{% endhighlight %}

> Nota: É de suma importância que a senha do --adminpass seja segura (com números,letras e caracteres especiais). Caso contrário, retornará um erro e não irá promover o samba. 
...
# Para o terceiro e último comando
...
No Cent0s é recomendado que antes de qualquer coisa, você desative o SELINUX:

{% highlight bash %}
vim /etc/selinux/config 

# onde tem enforcing, substitua por

SELINUX=disable
{% endhighlight %}

Desative o firewall caso não tenha alguma implicação e\ou não estiver usando-o

{% highlight bash %}
systemctl stop firewalld.service && systemctl disable firewalld.service
{% endhighlight %}

Faça o backup do smb.conf por segurança:

{% highlight bash %}
mv /etc/samba/smb.conf /etc/samba/smb-default.bak
{% endhighlight %}
...
Existe uma ferramenta bastante útil do samba chamada **testparm** que faz a verificação de erros no arquivo smb.conf e além disso, monta uma configuração nova em caso de novo arquivo smb.conf:

{% highlight bash %}
testparm 
{% endhighlight %}

Em seguida, crie dois diretórios pertencentes ao netlogon e profiles no samba:
...
> Nota: Você deve estar se perguntando o que significa este -m 1777 certo? -m é um parâmetro de "mode" para atribuir permissões tipo "chmod". Já a permissão 1777, este "1" a frente significa bitstickc. Em termos prático, quer dizer exatamente isto **não deixe ninguém excluir esta pasta ou mudar a permissão".

---

# Compartilhamento com o servidor de impressão

Você pode compartilhar as impressoras já configurados com o CUPS, tenha em mente que o Samba comunica com o CUPS via soquetes, portanto, você não precisa configurar qualquer permissão especial, além de uma diretiva Listen para o socket CUPS.

Crie um diretório de spool de impressão, e definir as permissões corretamente. Este é onde o Samba irá armazenar arquivos temporários relacionados para imprimir documentos:
...
Configurar o samba para usá-lo, adicionando o seguinte para /usr/local/samba/etc/smb.conf:
...
Drivers de ponto e impressão

Por uma questão de conveniência, os clientes do Windows podem consultar o servidor que está compartilhando uma impressora para um driver de impressão. Para habilitar essa funcionalidade no Samba, temos de criar uma impressora especial de compartilhamento de arquivos.

Criar um diretório de compartimento de impressão, e arquitetura de sub-diretórios:

{% highlight bash %}
mkdir -p /usr/local/samba/var/print/{COLOR,IA64,W32ALPHA,W32MIPS,W32PPC,W32X86,WIN40,x64}
{% endhighlight %}

Configurar no smb.conf.
...
A partir de agora efetue login como um administrador de domínio em um computador cliente Windows e clique em Iniciar -> Executar "\\samba\". Na lista de ações, clique duas vezes "Impressoras e faxes" e em seguida, clique em Arquivo -> Propriedades do Servidor. Na guia Drivers, clique em "Adicionar …", em seguida, em "Next".
