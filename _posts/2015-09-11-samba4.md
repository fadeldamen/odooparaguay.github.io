...
mensagens e o Gerenciador de Contas de Segurança. (RUSSINOVICH, SOLOMON e ALLCHIN, 2004, p. 840).

No Active Directory são encontrados objetos, florestas, arvores, domínios, partiçoes, esquemas e unidades organizacionais (similares a pastas), que são explicados a seguir.No Active Directory são encontrados objetos, florestas, arvores, domínios, partiçoes,esquemas e unidades organizacionais (similares a pastas), que são explicados a seguir. Uma estrutura de AD é um arranjo de informações sobre objetos. Os objetos se dividem em duas categorias principais (TECHNET ARTICLES, 2013):
