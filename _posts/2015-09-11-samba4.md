...
### Requisitos e Preparação do Sistema Operacional
...
### Configurando e instalando o Samba com A.D
...
> Nota: Deixe as respostas padrão na configuração do Kerberos. Apenas atentando para a primeira pergunta "Reino por omissão do Kerberos", informe seu domínio em letras maiúsculas.

{% highlight bash %}
sudo apt-get install -y samba smbclient
{% endhighlight %}

> Nota: A instalação do **smbclient** é opcional, utilizaremos apenas para testar as configurações adiante.
...
Caso não saiba como configurar seu **smb.conf**, consulte o site **[http://www.sloop.net/smb.conf.html](http://www.sloop.net/smb.conf.html)**.

Depois de tudo a cima feito, vamos testar as configurações. Verifique versão do samba e smbclient:
