...
{% highlight Text %} Qualquer sistema de arquivos ext3 existente pode ser migrado para o Ext4 com um procedimento fácil, que consiste na execução de um par de comandos em modo de somente leitura (descrito na próxima seção). Isso significa que você pode melhorar o desempenho, limites de armazenamento e recursos de seu sistema de arquivos atuais sem reformatar e / ou reinstalar o sistema operacional e ambiente de software. Se você precisar as vantagens do Ext4 em um sistema de produção, você pode atualizar o sistema de arquivos.
